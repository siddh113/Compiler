...
namespace Compiler.IO
{
    ...
    public class FileReader : IFileReader
    {
        ...
        /// <summary>
        /// Stores previously read lines for MoveBack()
        /// </summary>
        private Stack<string> previousLines = new Stack<string>();
        ...
        {
            if (currentPositionInLine > 1)
            {
                // Move back within the current line
                currentPositionInLine--;
            }
            else if (previousLines.Count > 0)
            {
                // Move back to the previous line
                currentLine = previousLines.Pop();
                currentLineNumber--;
                currentPositionInLine = currentLine.Length > 0 ? currentLine.Length - 1 : 1;
            }
        }
