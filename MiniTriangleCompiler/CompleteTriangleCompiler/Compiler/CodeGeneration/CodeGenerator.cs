...
namespace Compiler.CodeGeneration
{
    ...
    public class CodeGenerator
    {
        ...
        /// <summary>
        /// Generates code for a Quick If Command (? <expression> => <command>)
        /// </summary>
        private void GenerateCodeForQuickIfCommand(QuickIfCommandNode quickIfCommand)
        {
            Debugger.Write("Generating code for Quick If Command");
            ...
            GenerateCodeFor(quickIfCommand.Command);
            code.PatchInstructionToJumpHere(jumpAddress);
        }

        /// <summary>
        /// Generates code for a Loop Command (loop <command> while ( <expression> ) <command> repeat)
        /// </summary>
        private void GenerateCodeForLoopCommand(LoopCommandNode loopCommand)
        {
            Debugger.Write("Generating code for Loop Command");

            Address startAddress = code.NextAddress;
            GenerateCodeFor(loopCommand.InitialCommand);

            Address conditionCheckAddress = code.NextAddress;
            GenerateCodeFor(loopCommand.Expression);
            Address exitJumpAddress = code.NextAddress;
            code.AddInstruction(OpCode.JUMPIF, Register.CB, FalseValue, 0);

            GenerateCodeFor(loopCommand.LoopCommand);
            code.AddInstruction(OpCode.JUMP, Register.CB, FalseValue, 0);
            code.PatchInstructionToJumpHere(exitJumpAddress);
        }
